...
local function SetTabs(frame, numTabs, ...)
	...
	for i = 1, numTabs do
		local tab = CreateFrame("Button", frameName.."Tab"..i, frame, "CharacterFrameTabButtonTemplate"); --changed to CharacterFrameTabButtonTemplate
		tab:SetID(i);
		...
		if tabs[i] == "Interrupt" then
			...
			tab.content.input:SetMaxLetters(32)
			tab.content.input:SetPoint("TOPLEFT", tab.content, "TOPRIGHT", 115, 2)
			...
			tab.content.add:SetPoint("TOPLEFT",	tab.content.input, "TOPRIGHT", 0, 0)
		...
		if tabs[i] == "Interrupt" then
			tab.content.reset:SetPoint("CENTER", tab.content,  "CENTER", 915, 245 )
		else
			tab.content.reset:SetPoint("CENTER", tab.content,  "CENTER", 915, 214 )
		end
		...
		if (i == 1) then
			...
			if rowCount <= 9 then
				tab:SetPoint("TOPLEFT", _G[frameName.."Tab"..(i - 1)], "TOPRIGHT", -22, 0);
				rowCount = rowCount + 1
	    	else
				y = 7 - (25 * rows)
	...
	for l = 1, #L.spells[1][i] do
		...
		if (spellID) then
			...
			if spellCheck then
				...
				spellCheck.text = spellCheck:CreateFontString(nil, "ARTWORK")
				spellCheck.text:SetParent(spellCheck)
				spellCheck.text:SetTextColor(1, 1, 1, 1)
		
				spellCheck.text:SetJustifyH("RIGHT")
				spellCheck.text:SetPoint("LEFT", spellCheck, "RIGHT", 0, -1)
				spellCheck.text:SetFont(STANDARD_TEXT_FONT, 10, "OUTLINE")
				...
				if type(spellID) == "number" then
					if duration then
						if GetSpellInfo(spellID) then 
							aString = GetSpellInfo(spellID)..": "..duration or "REMOVED: "..spellID
						else
							aString = GetSpellInfo(spellID) or "REMOVED: "..spellID
						end
					elseif GetSpellInfo(spellID) then 
						aString = GetSpellInfo(spellID) or "REMOVED: "..spellID
					else
						aString = "REMOVED: "..spellID
					end
					spellCheck.icon:SetNormalTexture(GetSpellTexture(spellID) or 1)
				else
					spellCheck.icon:SetNormalTexture(1120721)
